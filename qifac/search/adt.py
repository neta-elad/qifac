...
class Problem:
    ...
    def solve(self, initial_models: List["Model"]) -> None:
        solver = ProblemSolver(self, initial_models)
        print(f"\nTrying to do MBQI:\n")
        problem_solver = solver.problem_solver
        solver.solve()
...
class ProblemSolver:
    ...
    def terms_for_instantiation(self) -> List[z3.Const]:
        return [
            z3.Const(f"t_{i}", self.problem.ground_term_adt) for i in range(self.problem.n_terms)
        ]
    ...
    def adt_solver(self) -> z3.Solver:
        ...
        for model in self.initial_models:
            ...
            for i, v in enumerate(violated):
                if v:
                    ...
                    g = z3.substitute_vars(self.problem.forall_assertions[i].body(), *ts)
        ...
        print(
            ...
            [self.problem.adt_to_term(adt_model.eval(t)) for t in self.terms_for_instantiation],
        )
        print(
        ...
        while (result := self.problem_solver.check()) != z3.unsat:
            ...
            if result == z3.sat:
                ...
                for model in self.models:
                    ground_instantiations.append(self.get_instantiation(new_adt_model, model))
            ...
            print(
                f"\nThey correspond to terms:",
                [self.problem.adt_to_term(new_adt_model[t]) for t in self.terms_for_instantiation],
            )
            print(
                f"\nAll models are hit using the following {len(ground_instantiations)} ground instantiations:"
            )
