...
forall_assertions = [
    ...
    z3.ForAll(
        [v, n],
        z3.Implies(decided(v), z3.Implies(member(n, quorum_of_decided(v)), vote(n, v))),
    ),
...
GroundTerm.declare(
    "GT_intersection",
    ("GT_intersection_0", GroundTerm),
    ("GT_intersection_1", GroundTerm),
)
...
def adt_to_term(t):
    ...
    if _(GroundTerm.is_GT_v1):
        ...
        return intersection(
            _(GroundTerm.GT_intersection_0), _(GroundTerm.GT_intersection_1)
        )
...
def add_model(I, M):
    ...
    for c, ci in zip(constants, constant_interps):
        entries.append((getattr(GroundTerm, f"is_GT_{c}")(t), ES[ci]))
    for f, fi in zip(functions, function_interps):
        for xs in product(range(len(U)), repeat=f.arity()):
            entries.append(
                (
                    z3.And(
                        getattr(GroundTerm, f"is_GT_{f}")(t),
                        *(
                            interpret(getattr(GroundTerm, f"GT_{f}_{i}")(t))
                            == ES[xs[i]]
    ...
    indicators = [
        z3.Bool(f"violate_{I}_{i}", ctx=ctx) for i in range(len(forall_assertions))
    ]
    witnesses = [
    ...
    for f, b in zip(forall_assertions, bodies):
        ...
        for xs, es in zip(product(U, repeat=n), product(ES, repeat=n)):
            res = to_bool(
                M.eval(z3.substitute_vars(f.body(), *xs), model_completion=True)
            )
            lit = b(*es)
            if not res:
                lit = z3.Not(lit)
    ...
    for i in range(len(forall_assertions)):
        ...
        for w in witnesses[i]:
            s.add(
                z3.Implies(
                    indicators[i],
                    z3.Or(*(w == interpret(t) for t in terms_for_instantiation)),
                )
            )
    ...
    for (I, (M, U, (E, ES), interpret, bodies, indicators, witnesses)) in enumerate(
        ...
        for i, v in enumerate(violated):
            if v:
                ...
                for w in ws:
                    eqs = [
                        to_bool(z3.simplify(w == m.eval(interpret(m[t]))))
                ...
                assert [U.index(e) for e in elems] == [
                    ES.index(m[w]) for w in witnesses[i]
                ]
...
while (res := s_smt.check()) != z3.unsat:
    ...
    print(
        ...
        [adt_to_term(m[t]) for t in terms_for_instantiation],
    )
    print(
        f"\nAll models are hit using the following {len(ground_instantiations)} ground instantiations:"
    )
